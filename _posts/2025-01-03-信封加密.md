...
from typing_extensions import IO, Optional, Self, Set, Tuple, Union
...
class EncryptedMessage:
    ...
    def serialize(self) -> str:
        ...
        """
        return json.dumps(
            {k: base64.b64encode(v).decode() for k, v in self.__dict__.items() if v is not None},
            separators=(",", ":"),
        )
...
class ResponseKey:
    ...
    def decrypt(self, response: Union[str, bytes]) -> bytes:
        ...
        if message.ciphertext is None:
            raise error.ParamError("response", "Message has no data")
...
class ServerSessionKey:
    ...
    @classmethod
    def load(
        cls, private_pem: Union[str, bytes], expire_at: datetime, tks_version: Optional[int]
    ) -> Self:
        """
        ...
        return cls(private_key, expire_at, tks_version)
    ...
    def generate(cls, expire_at: datetime) -> Self:
        ...
        return cls(private_key, expire_at, tks_version=None)
    ...
    def is_expired(self) -> bool:
        ...
        if now > self.expire_at:
            logger.warning(f"session key maybe expired")
    ...
    def decrypt(self, message: Union[str, bytes, EncryptedMessage]) -> Tuple[bytes, ResponseKey]:
        """
        信封解密.
        ...
        Raises:
            KeyExiredError: 密钥已超过有效期.
            DecryptionError: 加密消息不合法, 或者加密使用的密钥不是本密钥.
            NonceReusedError: 同一条消息已在之前解密过.
        """
